...
export function skyLook(conditions: Conditions): SkyLook {
  ...
  if (night > 0.9) {
    ...
    return {
      ...
      fog: { color: "#1a2a4a", density: 0.00042 },
    };
  }
  ...
  return {
    ...
    sunIntensity: daylight * mix(1.2, 0.48, overcast),
    sunColor: mixHex("#ffb46b", "#fff4e0", golden),
    fillIntensity: mix(0.32, 0.4, overcast),
    fillColor: mixHex("#ffe6bd", "#dfe8f5", overcast),
    ambient: 0.36 + overcast * 0.22 + night * 0.14,
    hemiSky: mixHex(
      mixHex("#cfe6ff", "#b7c0ca", overcast),
      "#5a5f7a",
      night * 0.7,
    ),
    hemiGround: mixHex("#5d8f4d", "#38512f", Math.max(overcast * 0.5, night * 0.6)),
    hemiIntensity: mix(0.95, 0.75, overcast),
    turbidity: mix(2.8, 13, overcast) + (1 - golden) * 3,
