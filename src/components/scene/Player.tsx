...
/** Ease one pose toward another, channel by channel. */
function blendPose(from: PoseValues, to: PoseValues, k: number): PoseValues {
  const out = { ...from };
  for (const key of Object.keys(out) as (keyof PoseValues)[]) {
...
function swingPose(t: number, batSide: "R" | "L", missed: boolean): PoseValues {
  ...
  // A miss ends with him getting back on his feet. Without it the pose would
  // hold the corkscrew until the idle pass took it away, and hand over to the
  // stance in one frame from the far side of a full turn.
  return recover > 0 ? blendPose(swung, battingStance(batSide), 0.72 * recover) : swung;
}
